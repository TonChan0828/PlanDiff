...
初版はマーケティングサイト(https://structured.app/)のヒーローを採寸し、
`--paper` にコーラル `#faeae7` を採用した。**これは誤りだった** —
コーラル一色の背景はLPの演出で、**アプリ本体は真逆**(クリームのクローム + 白カード +
コーラルのアクセント)である。ユーザー指摘を受け、アプリ本体を採寸し直した。

| 項目 | マーケティングサイト(初版・誤) | **アプリ本体(採用)** |
| --- | --- | --- |
| 背景 | コーラル `#FFA7A0` / 下層 `#F8DDD9` | **クリーム**(クローム)+ **白**(タイムライン面) |
| アクセント | スレートブルー `#6185A8` | **コーラル `#F88E87`**(今日マーカー・FAB・空き時間の注意喚起) |
| 本文 | 黒 | `#141414` |
| 淡色 | — | `#858585`(**暖色に振らない中間グレー**) |
| 副次色 | — | 紺・緑のカテゴリバッジ |
| フォント | Inter | Inter(同一) |
| 角丸 | ボタン `100px`、カード `32–40px` | ボタン 完全なピル、入力 `10px` |

アプリ本体の値は、Web版 (https://web.structured.app/) のログイン画面で
`getComputedStyle` により計測した(アカウント作成は行っていない)。

### そのまま使えない値

| アプリの値 | PlanDiff での問題 | 採用値 |
| --- | --- | --- |
| アクセント `#F88E87` | 白文字とのコントラスト **2.28:1**。`--brand` は実績ブロックのベタ塗り背景でもあり必ず白文字が乗る | `#b94e45`(**4.95:1**) |
| 淡色 `#858585` | クリーム背景で **3.33:1** | `#6b6b6b`(**4.94:1**) |

さらに `--brand` が赤系になった副作用として、その上に重なる**現在時刻ライン**(`--danger`)が
沈むリスクが出た。明度差が取れる深いクリムゾン `#6b0f2a` を採用している(`--brand` との明度差 **2.45:1**、S18で検証)。
...
コーラルの割り当ては**実績・ボタン側**とする(2026-08-29 ユーザー選択)。
Structured の顔色であるコーラルを PlanDiff の主要面に出し、`--interrupt` には
アプリのカテゴリバッジの紺を当てて色相で分離する。

| トークン | 値 | 色相 / 彩度 | 意図 |
| --- | --- | --- | --- |
| `--paper` | `#f9f7f3` | — | クリーム(アプリのクローム部分) |
| `--surface` | `#ffffff` | — | 白カード(アプリのタイムライン面) |
| `--ink` | `#33302e` | — | 柔らかい炭色(純黒を避ける) |
| `--ink-muted` | `#726b66` | — | 温かみのある淡色 |
| `--line` | `#ece7df` | — | クリーム寄りの罫線 |
| `--brand` | `#a05f58` | 6° / 29% | くすんだテラコッタ(実績・ブランド) |
| `--brand-ink` | `#ffffff` | — | |
| `--plan-fill` | `rgba(160,95,88,0.09)` | — | 予定 = 薄いテラコッタ |
| `--plan-border` | `rgba(160,95,88,0.3)` | — | |
| `--plan-text` | `#7d4c46` | 7° / 28% | テラコッタの文字用 |
| `--interrupt` | `#5a7590` | 210° / 23% | ダスティブルー(割り込み・ズレ) |
| `--danger` | `#5b3049` | 325° / 31% | 深いプラム(`--brand` と色相・明度の両方で分離) |
| `--success` | `#4a8168` | 153° / 27% | セージグリーン |
| `--hatch` | `rgba(90,117,144,0.45)` | — | ズレの斜線(`--interrupt` の45%) |
| `--grid` | `rgba(51,48,46,0.035)` | — | 方眼を弱める(アプリは罫線をほぼ使わない) |
| `--grid-hour` | `rgba(51,48,46,0.08)` | — | |
| `--focus` | `#a05f58` | — | |

### 柔らかさと色被りの解消(2026-08-29 ユーザー指摘)

初版のパレットに対して「もう少し柔らかい色味にしたい」「色被りが在って気になる」との
指摘を受けて改訂した。

**柔らかさ**: 白文字との AA(4.5:1)があるため**明度は上げられない**。
そのため「明るくする」のではなく「**彩度を下げる**」で柔らかさを出している
(彩度 35–75% → **23–31%**)。`--ink` も純黒 `#141414` から柔らかい炭色へ変えた。

**色被り**: 原因は `--brand` と `--danger` の**色相差が 23° しかなかった**こと。
コーラルが `--brand` を占めたことで `--danger` も赤系に押し込まれ、カレンダー上で
予定の薄塗り・実績・現在時刻ラインが全て赤系に見えていた。
`--danger` を深いプラム(325°)へ移し、**色相差 41°** を確保した。

なお `--brand` をダスティブルーにして `--interrupt` をコーラルにする案も検討したが、
今度は `--interrupt` と `--danger` が 20° に接近して**問題が移動するだけ**だったため採らなかった。

値の導出理由は「参考元から実測したデザイン言語」の §そのまま使えない値 を参照。
上表は S14・S18・S19・S20 が通る範囲で実装時に調整してよい。
...
- **S18** [単体・境界値]: Given Structured の `--brand` と `--danger` / When 明度差を計算 /
  Then 2:1 以上。**理由**: アプリのアクセントがコーラルのため両者とも赤系になった。
  現在時刻ライン(`--danger`)は実績ブロック(`--brand` のベタ塗り)の上に重なるので、
  色相だけでなく明度でも分離していないと線が沈む

- **S19** [単体・境界値]: Given Structured の `--brand` / `--interrupt` / `--danger` / `--success` /
  When 各ペアの色相差を計算 / Then 35° 以上。**理由**: 意味の違う役割が同じ色相帯に入ると
  「色被り」として認識される。初版は `--brand` と `--danger` が 23° しか離れておらず、
  カレンダー上で予定の薄塗り・実績・現在時刻ラインが全て赤系に見えた
- **S20** [単体・境界値]: Given Structured の色トークン / When 彩度を計算 / Then 40% 以下。
  **理由**: 白文字とのAAがあるため明度は上げられない。彩度を抑えることで柔らかさを出している
