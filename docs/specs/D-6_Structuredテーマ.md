...
| トークン | 値 | 意図 |
| --- | --- | --- |
| `--paper` | `#f8f6f2` | クリーム(アプリのクローム部分) |
| `--surface` | `#ffffff` | 白カード(アプリのタイムライン面) |
| `--ink` | `#141414` | アプリの本文色そのまま |
| `--ink-muted` | `#6b6b6b` | アプリの `#858585` をAA通過まで暗く |
| `--line` | `#e8e4dc` | クリーム寄りの罫線 |
| `--brand` | `#b94e45` | 深いコーラル(実績・ブランド) |
| `--brand-ink` | `#ffffff` | |
| `--plan-fill` | `rgba(185,78,69,0.1)` | 予定 = 薄いコーラル |
| `--plan-border` | `rgba(185,78,69,0.35)` | |
| `--plan-text` | `#8f3a33` | コーラルの文字用 |
| `--interrupt` | `#3d5a80` | 紺(アプリのバッジ色。割り込み・ズレ) |
| `--danger` | `#6b0f2a` | 深いクリムゾン(`--brand` と明度で分離する) |
| `--success` | `#2f7d4f` | 緑(アプリのバッジ色) |
| `--hatch` | `rgba(61,90,128,0.5)` | ズレの斜線(`--interrupt` の50%) |
| `--grid` | `rgba(20,20,20,0.04)` | 方眼を弱める(アプリは罫線をほぼ使わない)。暖色に振らない |
| `--grid-hour` | `rgba(20,20,20,0.09)` | |
| `--focus` | `#b94e45` | |

値の導出理由は「参考元から実測したデザイン言語」の §そのまま使えない値 を参照。
上表は S14・S18 が通る範囲で実装時に調整してよい。
